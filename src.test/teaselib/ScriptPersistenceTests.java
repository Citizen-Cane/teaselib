...
public class ScriptPersistenceTests {
    ...
    public void testScriptPersistenceStringItemStorage() throws IOException {
        try (TestScript script = new TestScript()) {
            ...
            Assertions.assertTrue(script.storage.containsKey(key));
        }
    }
    ...
    public void testScriptNamespacePersistence() throws IOException {
        try (TestScript script = new TestScript()) {
            Assertions.assertNull(script.teaseLib.persistence
            ...
            Assertions.assertTrue(scriptSettings.isPresent());
            Assertions.assertTrue(scriptSettings.get().has(QualifiedName
                    .of(TeaseLib.DefaultDomain, script.namespace, "My_Test_Value_set_by_name").toString()));
            Assertions.assertEquals("Saved as local enum by name", script.teaseLib.persistence
                    .get(QualifiedName.of(TeaseLib.DefaultDomain, script.namespace, TestValuesEnumClass.My_Test_Value_set_by_name.name())));
        }
    }
    ...
    public void testGlobalPersistence() throws IOException {
        try (TestScript script = new TestScript()) {
            Assertions.assertNull(script.teaseLib.persistence.get(QualifiedName.of(TeaseLib.DefaultDomain, TestValuesEnumClass.My_Test_Value_set_by_enum)));

            script.teaseLib.persistence.set(QualifiedName.of(TeaseLib.DefaultDomain, TestValuesEnumClass.My_Test_Value_set_by_enum),
                    "Saved by local enum");
            ...
            Assertions.assertTrue(script.storage.containsKey(expected));
        }
    }
    ...
    public void testItemAvailability() throws IOException {
        try (TestScript script = new TestScript()) {
            ...
            Assertions.assertTrue(script.storage.containsKey(QualifiedName.of(TeaseLib.DefaultDomain,
    ...
    public void testDomainSeparation() throws IOException {
        try (TestScript script = new TestScript()) {
            ...
            Assertions.assertTrue(script.item(Clothes.Underpants).isAvailable());
            // TODO Support domain names for user items
            Assertions.assertFalse(script.domain(Clothes.Doll).item(Clothes.Underpants).isAvailable(), "Item domains not separated");

            script.domain(Clothes.Doll).item(Clothes.Underpants).setAvailable(true);
            Assertions.assertTrue(script.domain(Clothes.Doll).item(Clothes.Underpants).isAvailable());
    ...
    public void testSetVersusItem() throws IOException {
        try (TestScript script = new TestScript()) {
            ...
            Assertions.assertEquals(0, script.storage.size());

            item.setAvailable(true);
            Assertions.assertEquals(1, script.storage.size());
            Assertions.assertTrue(item.isAvailable());
            Assertions.assertTrue(script.storage.containsKey(qualified));
            ...
            Assertions.assertEquals(true, persistentBoolean.value());

            item.setAvailable(false);
            Assertions.assertEquals(1, script.storage.size());
            Assertions.assertFalse(item.isAvailable());
            Assertions.assertEquals(false, persistentBoolean.value());
        }
    }
    ...
    public void testScriptPersistencenGlobalNamespace() throws IOException {
        try (TestScript script = new TestScript()) {
            ...
            script.teaseLib.persistence.set(QualifiedName.of("My domain", TestValuesEnumClass.My_Test_Value_set_by_enum), "Saved by global enum");
            Assertions.assertTrue(script.storage.containsKey(QualifiedName.of("My domain",
                    "ScriptPersistenceTests.TestValuesEnumClass", "My_Test_Value_set_by_enum")));
    ...
    public void testPersistentEnum() throws IOException {
        try (TestScript script = new TestScript()) {
            ...
            PersistentEnum<TestValuesEnumClass> testValue = script.persistence.newEnum("test", TestValuesEnumClass.class);
            Assertions.assertFalse(testValue.available());

            for (TestValuesEnumClass value : TestValuesEnumClass.values()) {
                testValue.set(value);
                Assertions.assertEquals(value, testValue.value());
                script.storage.printTo(logger);
            }

            Assertions.assertTrue(testValue.available());
            testValue.clear();
            Assertions.assertFalse(testValue.available());
        }
    }
